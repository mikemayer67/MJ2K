...
class Pulse:
    ...
    def phi(self,t):
        return np.sin(2*np.pi*(0.5*self.Brf*t*(t-1) + self.n*t))
 
    def signal(self,t,x):
        return self.phi(t-x/self.n)
    ...
    def echo(self,t,x,tgt):
        return self.phi(t-(2*tgt.X-x)/self.n)
...
class PulseAnimation:
    def __init__(self, name, n=8, Brf=7, targets=[], nsamp=100, figsize=(15,2),
                composite=False):
        """
        Args:
            ...
            - composite (bool): add echos together rather than overlaying them
        """
        self.name = name
        self.n = n
    ...
    def gen_frames(self, nframes=128):
        ...
        for frame in range(nframes):
            ...
            if echos:
                ...
                if self.composite:
                    xo = min(e['xo'] for e in echos)
                    xe = max(e['xe'] for e in echos)
                    echo_x = np.arange(xo, xe, dx)
                    echo_y = sum( np.array(
                        [self.pulse.echo(t,x,e['tgt']) 
                         if x >= e['xo'] and x <= e['xe']
                         else 0
                         for x in echo_x])
                        for e in echos )
                    ax.plot(echo_x, echo_y, 'yellow')
                else:
                    for echo in echos:
...
def gen_anim(name):
    ...
    pa_args = dict()
    for arg in ('n','Brf','nsamp','figsize','composite'):
        if arg in pa_set:
